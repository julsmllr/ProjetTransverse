...
def verifCoordonnes(self, pos_balle_x, pos_balle_y, pos_panier, indice_position):
    if pos_panier[0] < pos_balle_x[indice_position] < pos_panier[0] + 100:
        if (pos_panier[1] - 25 < pos_balle_y[indice_position] < pos_panier[1]) and (
                pos_balle_y[indice_position - 10] < pos_balle_y[indice_position]):
            return True
        else:
            return False
    else:
...
def dessinBallonNonLancer(self):
    posBalleX, posBalleY, posRebond = calculate_trajectory(self.power, self.angle, (self.width, self.height),
                                                           position_initiale=self.pos_joueur)
